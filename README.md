...
Perímetro (WAF): Imagine uma cerca ao redor do site. O WAF (Web Application Firewall) é como essa cerca, que impede que ataques comuns (como tentar roubar dados ou causar falhas) cheguem até ele.
Identidade (IAM): Isso é como garantir que somente as pessoas certas possam entrar no sistema. IAM (Identity and Access Management) é um sistema que gerencia quem pode acessar o quê no seu site ou banco de dados, como se fosse um controle de quem pode entrar em um prédio.
Monitoramento Centralizado (SIEM): É como ter câmeras de segurança em todos os cantos da sua casa, monitorando tudo o que acontece. O SIEM coleta todos os "registros" (ou logs) do seu sistema e os coloca em um lugar único, facilitando a detecção de problemas e ataques.
Hardening: É o processo de "fortificar" o sistema, tirando coisas desnecessárias e deixando apenas o que é realmente necessário para o funcionamento. Como tirar as brechas de uma casa para que ladrões não entrem.
Controle de Acesso: Se refere a garantir que apenas as pessoas certas (como funcionários ou usuários) tenham acesso a informações sensíveis. Por exemplo, apenas você tem a chave do cofre.
Runbooks Simplificados de Resposta a Incidentes: São instruções claras de como reagir rapidamente quando algo de errado acontece. Como um manual que você segue quando a casa pega fogo, para saber o que fazer para minimizar os danos.
Ganhos Esperados durante o processo
Redução do Tempo de Detecção e Resposta (MTTD/MTTR): Significa que, quando algo errado acontece, conseguimos perceber mais rapidamente e agir para corrigir. Ou seja, detectar e resolver problemas mais rápido.
Aumento da Proteção Contra Ataques Automatizados: Hackers usam ferramentas automáticas para atacar sistemas. Com as camadas de segurança, aumentamos as chances de impedir esses ataques automáticos antes que eles causem danos.
Melhoria na Postura de Segurança com Ações Viáveis em Até 30 Dias: Vamos melhorar a segurança de maneira simples e rápida, em um prazo de 30 dias, implementando as ações que trazem resultados reais no curto prazo.
Maior Controle, Rastreabilidade e Visibilidade de Incidentes: Vai ser mais fácil ver tudo o que acontece no sistema e entender o que há de errado, ajudando a evitar que o problema aconteça de novo. Como ter uma visão clara de tudo que acontece na sua casa.
...
Próximos Passos
Aprovação da proposta e orçamento.
Execução das ações de 30 dias (WAF, MFA, logs centralizados).
Treinamento da equipe sobre os procedimentos de segurança e resposta a incidentes.
Acompanhamento contínuo e melhorias progressivas no monitoramento e infraestrutura.
